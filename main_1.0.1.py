...
def main():
    ...
    print("큐시트생성기 1.0.1 for Windows\n")
    ...
    except AttributeError: # Windows가 아닌 환경
...
def make_dir():
    ...
    try:
        ...
        #print(f"[Info] '{new_dir_path}' 디렉토리가 성공적으로 생성되었습니다.")
        return new_dir_path
    except OSError as e:
    ...
    if ext.lower() == '.mp3':  
        try:
            ...
            if 'TIT2' in audio.tags:
                 try:
                     # UTF-8로 시도
                     # metadata['title'] = audio.tags['TIT2'].text[0].encode('latin-1').decode('utf-8', errors='ignore').strip()
                     # CP949로 시도 (Windows 한글 환경)
                     metadata['title'] = audio.tags['TIT2'].text[0].encode('latin-1').decode('cp949', errors='ignore').strip()
                 except Exception as decode_e:
                     print(f"[Warning] '{file_name}' 제목 디코딩 실패 ({decode_e}). 원본 값 사용.")
                     metadata['title'] = str(audio.tags['TIT2'].text[0]).strip()

            if 'TPE1' in audio.tags:
                 try:
                     # UTF-8로 시도
                     # metadata['Artist'] = audio.tags['TPE1'].text[0].encode('latin-1').decode('utf-8', errors='ignore').strip()
                     # CP949로 시도 (Windows 한글 환경)
                     metadata['Artist'] = audio.tags['TPE1'].text[0].encode('latin-1').decode('cp949', errors='ignore').strip()
                 except Exception as decode_e:
                     print(f"[Warning] '{file_name}' 제목 디코딩 실패 ({decode_e}). 원본 값 사용.")
                     metadata['Artist'] = str(audio.tags['TPE1'].text[0]).strip()

            if 'TALB' in audio.tags:
                 try:
                     # UTF-8로 시도
                     # metadata['Album'] = audio.tags['TALB'].text[0].encode('latin-1').decode('utf-8', errors='ignore').strip()
                     # CP949로 시도 (Windows 한글 환경)
                     metadata['Album'] = audio.tags['TALB'].text[0].encode('latin-1').decode('cp949', errors='ignore').strip()
                 except Exception as decode_e:
                     print(f"[Warning] '{file_name}' 제목 디코딩 실패 ({decode_e}). 원본 값 사용.")
                     metadata['Album'] = str(audio.tags['TALB'].text[0]).strip()

            #metadata['title'] = str(audio.get('TIT2', [''])[0]).strip()
            #metadata['Artist'] = str(audio.get('TPE1', [''])[0]).strip()
            #metadata['Album'] = str(audio.get('TALB', [''])[0]).strip()
            ...
            metadata['Artist'] = "-"
            metadata['Album'] = "-"
            print(f"[Warning] '{file_name}' 파일에 메타데이터가 없습니다. 추후 직접 데이터를 입력해주세요.")
            ...
            print(f"[Error] '{file_name}' 메타데이터 추출 중 예외 발생: {e}")
        ...
        metadata['Artist'] = "-"
        metadata['Album'] = "-"
        print(f"[Warning] '{file_name}' 파일은 MP3 파일이 아닙니다. 추후 직접 데이터를 입력해주세요.")
